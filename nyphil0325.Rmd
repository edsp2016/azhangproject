...
---
Around the turn of the century, New York City became the arts center of the world. A major factor was the establishment and expansion of the New York Philharmonic which not only encouraged the flourishing of American musicians but also attracted musicians from all over the world to NYC. The other factor was NY Phil not only performed the famous classical pieces that the audiences are already familiar with but also establisthed programs like composers-in-residency to perform and promote living composers'works. In this research, I would like to study the performance history of the New York Philharmonic, analyze and visualize the changes of diversity in composers. I want to see with the rise of NYC as the center of art and culture, will there be a change from the strongly Euro-centric, older(deceased), male composers. 

#Getting data from NY Philharmonic's github page
...
To get a general sense of the data, I decide to ordern the composers by the number of works got performed from a descending order. 
```{r}
...
The following graph show that most of the composers works get performed less than ten times, and only 16 composers's works are performed more than 1000 times. Therefore, I expect the composers are very diversee. 
```{r}
nrow(SumComp1)
...
Because the number of performance increase year by year, I decide to compute a popularity score to even the number out. I sumed up the total number of performance in each seaon. I got the popularity score by dividing the number of performance for each composers in each season by the total number of performance in each season.
```{r,eval=FALSE}
require(base)
...
#Scraping wikipedia and get a list of woman composers:

I would like to know if the proportion of woman composers of total composers' works that are performed by NY philharmonic increase of time, but NY Philharmonic's data does not have gender information for each composers. Therefore, I scraped wikipedia page on a list of women composers (https://en.wikipedia.org/wiki/List_of_female_composers_by_birth_year) in python. I matched the name of women composers with the composers names in the popularity score table and computed the proportion of women composers's works performed in each season and graphed it. 
```{r}
library(png)
library(grid)
...
```
The graph shows that there is not much increase of the proportion of women composers over the total number of composers whose works are performed by NY Philharmonic. However, that does not say women are not becoming more important in the field of classical music. In Baroque, classical and romantical era, most of the composers are male, and the rise of women composers start relatively recently. Therefore, next step is to see the proportion of the number of women composers whose works get performed by NY Philharmonic over the number of living composers whoser works get performed by NY Philharmonic. 

# composers nationalities
I would like to see the components of nationalitiese of composers whose works are performed by NY Philharmonic. The NY Philharmonic data does not have the nationality of composers. Therefore, I scraped wikipedia page and got a lists of composers nationalities data.
...
I stacked the data from multiple pages and cleaned with and matched them with the popularity score table and computed the proportion of American composers whose works are performed by the NY Philharmonic over total number of works over time.
```{r}
...
# }
```
The graph shows a general increase of American composers' proportion over time, and the next step is to see the proportion of American composers over living composers over time. The taper down in the end might be the rise of Asian composers. 
...
# Scraping composer birth and death year. 
For this part, I am scraping wikipedia page as well. The web pages' structure for different time period is different. I scraped the classical era composer's birth and death year page (https://en.wikipedia.org/wiki/List_of_Classical-era_composers) in python with the following code.
```{r}
...
names=list(rep(0,length(classical1)))
for (i in 1:length(classical1)){
  names[i]=strsplit(classical1[[i]],"")[[i]][1]
}
```
...
```{r}
twentiethC <- read.csv("~/GitHub/azhangproject/twentiethC.csv", row.names=1,encoding="UTF-8")
#substitute NAs with current year

#find match. see if the composer appears in NY phil or not
for (i in 1: nrow(twentiethC)){
  twentiethC$Lname[i]=strsplit(as.character(twentiethC$Name1[[i]]),", ")[[1]][1]
  twentiethC$Fname[i]=strsplit(as.character(twentiethC$Name1[[i]]),", ")[[1]][2]
  twentiethC$Name2[i]=paste(twentiethC$Lname[i],twentiethC$Fname[i],sep=",  ")
}

twentiethCS=twentiethC[3:1963,c(2,6)]
names(twentiethCS)[names(twentiethCS)=="Year.of.death"]="death"
colnames(twentiethCS$Year.of.death)="death"
twentiethCS$death=as.character(twentiethCS$death)
twentiethCS$death[is.na(twentiethCS$death)]=2016

l=c()
for (i in 1:nrow(twentiethC)){
  l=c(l,which(twentiethC$Name[i]==popScoreComposerComplete$composers))
}
#find the col number of the first non 0 value and get the year
```

```{r}
comp=composerBySeasonComplete[1:175]
l=c()
...
```{r}
twentyfirstC <- read.csv("~/GitHub/azhangproject/twentyfirstC.csv", row.names=1,encoding="UTF-8")
```
