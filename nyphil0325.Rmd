...
american1.1=strsplit(american1," ")

l=c()
for ( i in 1:length(american1.1)){
  if(length(grep(american1.1[[i]][1],popScoreComposerComplete$composers))>0 && 
     length(grep(american1.1[[i]][length(american1.1[[i]])],popScoreComposerComplete$composers))>0){
    test=grep(american1.1[[i]][1],popScoreComposerComplete$composers)
    l=c(l,test)
     }
  }
l
