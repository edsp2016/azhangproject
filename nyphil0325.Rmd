...
qplot(seq_along(jewPopSum),jewPopSum)+geom_line()+ylim(0,1)+geom_area(colour="black")+  theme(axis.text.x = element_text(angle = 45,size=5, hjust = 1))+scale_x_discrete(labels=seq(1842,2015,by=2))
```
...
total=select(V)
total=na.omit(total)
#ggplot(total, aes(x=seasons,y=scores,color=V1,group=V1))+geom_area()
#qplot(total,aes(x=seasons,y=scores,color=V1,group=V1))+geom_area()
```
